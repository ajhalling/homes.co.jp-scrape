...
class TokyoSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for listing in listings:
            ...
            stories_tall_str = ""
            result = stories_tall_str.startswith("地")
            if result == True:
                stories_tall = list(stories_tall_raw)[-1]
                stories_tall1 =  stories_tall
                intcheck = isinstance(list(stories_tall_raw)[-2], int)
                if intcheck == True:
                    stories_tall2 = list(stories_tall_raw)[-2]
                    stories_tall = str(stories_tall2) + str(stories_tall1)
            else:
                if stories_tall_str == "平屋":
                    stories_tall = 1
                else:
